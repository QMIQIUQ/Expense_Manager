...
- **修改1**: 將觸發分支從 `main1` 改為 `main`
- **修改2**: 移除環境保護配置，避免部署被拒絕的錯誤
- **修改3**: 在 deploy job 中添加明確的權限設定
- **效果**: 恢復主分支自動部署到 GitHub Pages 的功能，解決環境保護規則錯誤
...
4. **環境保護規則**：
   - 如果看到 "Branch is not allowed to deploy" 錯誤，檢查儲存庫設定中的環境保護規則
   - 目前的配置已移除環境保護限制，避免部署被拒絕
